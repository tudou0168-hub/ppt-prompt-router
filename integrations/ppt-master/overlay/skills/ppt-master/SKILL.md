...
| Prompt Router handoff | [`references/prompt-router-integration.md`](references/prompt-router-integration.md) | Owns the Router 2.0 director contract, single director plan, and controlled-production activation |
...
| `${SKILL_DIR}/scripts/reference_elements.py` | Extract project-scoped visual language from a reference PPTX; always flat, never page filling |
| `${SKILL_DIR}/scripts/director_plan.py` | Validate and install the single `analysis/director_plan.json` |
| `${SKILL_DIR}/scripts/production.py` | Opt-in serial page production, invalidation, sample/review gates, and export preflight |
...
**Hard boundary — raw PPTX template references are not Step 3 templates.** PPTX-as-source remains valid in Step 1 / Step 2, and raw PPTX template + generated PPTX routes to `template-fill`. The single exception is a validated Router 2.0 contract with `template_intent: reference_elements`: `router-accept` has already extracted a project-scoped design system, locked `pptx_structure.mode: flat`, and forbidden source page/layout reuse. In that route, consume the project `design_spec.md` and `spec_lock.md`, skip style/font/palette/brand selection, and redesign every page. Otherwise, if the user wants the SVG/template-based generation route from that PPTX, stop before Step 3 and run [`workflows/create-template.md`](workflows/create-template.md).
...
**Router planning activation**: When this run entered through `ppt-prompt-router` 1.3.0, read [`references/prompt-router-integration.md`](references/prompt-router-integration.md) and keep the one selected professional director prompt active before planning starts. Use it while producing `content_map`, Storyline, page directory, one core conclusion per page, and `slide_plan`. These remain native Strategist artifacts and continue through the existing confirmation, `design_spec.md`, and `spec_lock.md` flow.
...
**Controlled-project gate**: If `analysis/production_state.json` exists, read [`references/prompt-router-integration.md`](references/prompt-router-integration.md) and use `production.py` for every page. Write only `.page_work/current.svg`; run `check-render`, open the latest iteration PNG, apply the existing [`visual-review`](workflows/visual-review.md) rubric, update `.review/Pxx.json`, repair and rerender when required, then call `pass-page`. Router 2.1 samples require two real iterations; ordinary pages require at least one. Direct writes to `svg_output/` are forbidden because only `pass-page` may promote a machine-checked, hash-bound, visually reviewed page.
...
**Visual Construction Phase**: legacy projects generate sequentially into `<project_path>/svg_output/`. Controlled projects generate sequentially through `.page_work/current.svg` and `production.py`; passed pages are promoted into `<project_path>/svg_output/`.

**Controlled mid-run gate**: At the recorded midpoint threshold, compare the three sample PNGs with the latest pages for layout copying, shrinking type, information crowding, visual regression, and drift. Record the result with `production.py review <project> midpoint ...`; the next page remains blocked until approval.
...
> **Visual self-check**: Router controlled projects already run the existing [`visual-review`](workflows/visual-review.md) workflow per page and must not run a duplicate engine here. For legacy projects only, the standalone whole-deck re-pass remains opt-in and runs when the user explicitly requests it.
...
**Controlled-project final gate**: When `analysis/production_state.json` exists, record the whole-deck review through `production.py review <project> deck ...` after opening the complete deck overview. Rework issue pages through `reopen-page`. `svg_to_pptx.py` refuses export until every page hash/check and all required reviews are current.
