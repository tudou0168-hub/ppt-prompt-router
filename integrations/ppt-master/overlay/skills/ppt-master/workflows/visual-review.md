...
> Native visual-review capability used in two modes: standalone post-generation review for legacy projects, and mandatory per-page report-only review for Router controlled projects.
>
...
This is opt-in for legacy projects. It is automatically active when a Router controlled project has both `analysis/production_state.json` and a Director Plan 2.0 `pages` array. Controlled mode is part of the production gate and does not wait for a separate user request.
...
- Legacy mode: Executor (SKILL.md Step 6) has finished all pages
- Controlled mode: one active page exists at `.page_work/current.svg` and `check-render` produced the latest iteration PNG
- `svg_quality_checker.py` has passed
- Post-processing (`finalize_svg.py`, `svg_to_pptx.py`) has **not** yet run
- Legacy mode only: the user has explicitly requested visual review
...
## Controlled Page Mode

Controlled mode reuses this workflow's renderer, rubric, and `.review/<page>.json`; it does not create a second review engine.

1. `production.py check-render` runs the static checker and delegates rendering to `visual_review.py`, producing `.preview/Pxx.iterN.png`.
2. An independent reviewer opens that PNG and applies the rubric in **report-only** mode. The reviewer must not edit `.page_work/current.svg`, rewrite content, or restructure the page.
3. The main Executor applies required changes to `.page_work/current.svg`, then runs `check-render` again.
4. The reviewer updates the same `.review/Pxx.json`, including per-iteration SVG/PNG hashes and findings.
5. `production.py pass-page` verifies the report and hashes before promotion.

Samples always use budget 2 and must have a genuine first-round finding and a visible correction. Ordinary pages use budget 1 unless a finding requires another pass. On hosts without an independent reviewer primitive, the main agent performs the same review sequentially after switching explicitly from Executor to Reviewer role and opening the PNG.

## When NOT to Run

- Legacy mode only: the project has no `svg_output/<page>.svg` files yet
...
- Legacy mode only: the user has not asked for it
...
- **Iteration budget**: legacy and ordinary controlled pages default to 1. Router controlled samples are fixed at 2 by Production; other high-stakes pages may opt into 2.
...
For legacy projects, enable this by setting iteration budget = 2 in the orchestrator prompt. Router controlled samples already require two hash-bound iterations and are rejected by `production.py pass-page` when the evidence is incomplete. Each added iteration roughly doubles render cost and triples token cost on the affected pages.
