...
### §0.2 Controlled-project override

When `analysis/production_state.json` exists and `analysis/director_plan.json` uses `pages`, review is mandatory and runs before promotion:

- `svg_path` is `.page_work/current.svg`, not `svg_output/<page>.svg`.
- `png_path` is the latest `.preview/Pxx.iterN.png` returned by `production.py check-render`.
- The reviewer is **report-only**. It records findings but never edits the SVG; the main Executor owns all content, structure, and SVG changes.
- The reviewer also checks the current page's `headline`, `key_message`, `relationship_type`, `visual_anchor`, `rhythm_role`, sample risk, and reference-template constraints from the existing project files.
- A structural or content issue is a blocking item for the Executor, not permission for the reviewer to redesign the page.
- Sample pages use exactly the required two-pass minimum; ordinary pages use one pass unless findings require another.
...
### §5.1 Controlled report binding

Controlled mode keeps the same `.review/<page>.json` evidence file and adds the fields Production needs for deterministic release:
  ...
  "iterations": [
    {
      ...
      "svg_hash": "...",
      "png_hash": "...",
      "findings": ["specific observed issue"]
    },
    {
      "iteration": 2,
      "svg_hash": "...",
...
For samples, iteration 1 must contain at least one genuine finding, iteration 2 must bind changed SVG and PNG hashes, and final status must be `fixed`. Production rejects missing, stale, self-declared, or one-pass sample reports. Full diagnostic detail remains in this native report and is never copied into `production_state.json`.
...
For controlled work, `visual_review.py <project> --work-file .page_work/current.svg --page-id Pxx --iteration N` uses the same Playwright backend and lock, writes `.preview/Pxx.iterN.png`, and does not require the live-preview server.
