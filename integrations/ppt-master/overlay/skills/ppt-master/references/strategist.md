...
### Router Director Plan 2.0 (Controlled projects)

When `analysis/director_contract.json` exists, fully read `analysis/director_profile.md` before planning. Reorganize the material according to that audience, communication job, title voice, required story beats, opening/closing tasks, evidence standard, and failure modes; never treat the source chapter order as the slide order.

Write the only `analysis/director_plan.json` with top-level `content_map`, `fact_boundary`, `storyline`, `pages`, and `sample_pages`. Each page contains only `page_id`, `headline`, `page_goal`, `page_role`, `key_message`, `relationship_type`, `visual_anchor`, `evidence_refs`, and `rhythm_role`. Keep fonts, colors, layouts, resource ids, and SVG implementation out of the plan.

Select three different non-cover/non-contents/non-closing/non-plain-text samples by risk, not convenience:

- `card_stack_risk`: most likely to collapse into repeated cards
- `relationship_density`: highest information density and relationship complexity
- `visual_signature`: strongest test of the project's distinctive visual language

For `government_strategy`, the whole Storyline must close the loop `status → problem → cause → gap → path → action`; one page cannot simultaneously explain problem, cause, solution, and benefit.
...
> - Not every page needs a chart. When a page's information structure matches a catalog entry, **use that template as a structural starting point** — keep the visualization type and core layout logic, then adapt composition, density, color, decoration, and accompanying elements to fit this deck's content and visual tone. If a catalog structure and a custom SVG fit equally well, prefer the real catalog resource. If no exact match exists, custom SVG is a first-class choice. What is forbidden is (a) generating without reading the catalog, (b) blind verbatim mimicry, and (c) forcing a near-match that changes the page's actual relationship.
>
> **Workflow**:
...
> 2. If still no fit: re-evaluate the relationship and information density, then use the most suitable custom SVG, table, or illustration. Do not force a close-but-wrong template.
