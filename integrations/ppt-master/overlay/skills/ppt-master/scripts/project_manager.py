...
""",
    ...
    reference = subparsers.add_parser(
        "reference-elements",
        help="Extract a reference PPTX's visual language into a flat project design system",
    )
    reference.add_argument("project_path", help="Project directory")
    reference.add_argument("pptx", help="Reference PPTX path")
    reference.add_argument("--force", action="store_true")

    director = subparsers.add_parser(
        "director-plan",
        help="Validate and install the project's single director_plan.json",
    )
    director.add_argument("project_path", help="Project directory")
    director.add_argument("plan", help="Director plan JSON path")
    director.add_argument("--force", action="store_true")

    router_accept = subparsers.add_parser(
        "router-accept",
        help="Validate a Router 2.0 director handoff and initialize controlled production",
    )
    router_accept.add_argument("project_path", help="Project directory")
    return parser
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.command == "reference-elements":
            from reference_elements import extract_reference_elements

            result = extract_reference_elements(
                args.project_path,
                args.pptx,
                force=args.force,
            )
        ...
        if args.command == "director-plan":
            from director_plan import install_director_plan

            result = install_director_plan(
                args.project_path,
                args.plan,
                force=args.force,
            )
        ...
        if args.command == "router-accept":
            ...
            if not contract_path.is_file() or not profile_path.is_file():
                raise ValueError("director_contract.json and director_profile.md are required")
            ...
            if set(contract) != allowed_keys:
                raise ValueError(
                    f"director contract keys must be exactly {sorted(allowed_keys)}"
                )
            if contract.get("schema_version") != "1.0":
                raise ValueError("unsupported director contract schema")
            ...
            if not expected_hash or actual_hash != expected_hash:
                raise ValueError("director profile SHA256 does not match contract")
            page_count = int(contract.get("page_count") or 0)
            if page_count < 1:
                raise ValueError("page_count must be positive")
            intent = contract.get("template_intent")
            if intent not in {"reference_elements", "native_fill", "reusable_template", "none"}:
                raise ValueError(f"unsupported template intent: {intent}")

            template_result = None
            if intent == "reference_elements":
                template_path = Path(str(contract.get("template_path") or "")).expanduser()
                if not template_path.is_file():
                    raise ValueError(f"reference template not found: {template_path}")
                from reference_elements import extract_reference_elements

                template_result = extract_reference_elements(project, template_path)

            from production import init_production

            stage = {
                "reference_elements": "director_pending",
                "none": "director_pending",
                "native_fill": "native_fill_pending",
                "reusable_template": "reusable_template_pending",
            }[intent]
            init_production(
                project,
                [f"P{index:02d}" for index in range(1, page_count + 1)],
                stage=stage,
            )
            result = {
                "accepted": True,
                ...
                "next_action": {
                    "reference_elements": "strategist_director_plan",
                    "none": "strategist_director_plan",
                    "native_fill": "template_fill_pptx",
                    "reusable_template": "create_template",
                }[intent],
            }
            if template_result:
                result["reference_elements"] = template_result
            print(json.dumps(result, ensure_ascii=False, indent=2))
            return 0
